    ...
    <div class="modal-dialog modal-dialog-centered modal-xl" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title">Plantilla Medicamento Médico</h6><button aria-label="Close" class="close" data-dismiss="modal"
                ...
                <div class="row">
                    ...
                    <div class="col-md-5">
                        <div class="form-group">
                            <label for="medico_id">Médico <span class="requerido">*</span></label>
                            <select class="form-control" id="medico_id"></select>
                        </div>
                    </div>
                    <div class="col-md-5">
                        <div class="form-group">
...
<script>
    ...
    function inicializarEditForm() {
        $.ajax({
                ...
                success: function (response) {
                    ...
                    if ($('#medico_id').find("option[value='" + data.medico.id + "']").length) {
                        $('#medico_id').val(data.medico.id).trigger('change');
                    } else {
                        var newOption = new Option(data.medico.lastname + ' ' + data.medico.name, data.medico.id, true, true);
                        $('#medico_id').append(newOption).trigger('change');
                    }
    ...
    function save() {
        ...
        if (errors.length > 0) {
            ...
            if (id == 0) {
                ajax_type = 'POST';
                ajax_url = '/api/plantillas/medicamentos-medicos';
            } else {
                ajax_type = 'PUT';
                ajax_url = '/api/plantillas/medicamentos-medicos/' + id;
                ajax_data['id'] = id;
            }
            $.ajax({
                type: ajax_type,
